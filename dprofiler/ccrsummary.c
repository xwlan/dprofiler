...
DIALOG_SCALER_CHILD CcrSummaryChildren[] = {
	{ IDC_LIST_CCR_SUMMARY, AlignRight, AlignBottom },
	{ IDC_BUTTON_CCR_SUMMARY, AlignBoth, AlignBoth },
};

DIALOG_SCALER CcrSummaryScaler = {
	{0,0}, {0,0}, {0,0}, 2, CcrSummaryChildren
};

LISTVIEW_COLUMN CcrSummaryColumn[] = {
	...
	)
{
	...
	Object->ResourceId = IDD_FORMVIEW_CCR_SUMMARY;
	Object->Procedure = CcrSummaryProcedure;
	...
	)
{
	...
	hWndCtrl = GetDlgItem(hWnd, IDC_LIST_CCR_SUMMARY);
	...
	for (i = 0; i < 2; i++) { 
        ...
        lvc.iSubItem = i;
		lvc.pszText = CcrSummaryColumn[i].Title;	
		lvc.cx = CcrSummaryColumn[i].Width;     
		lvc.fmt = CcrSummaryColumn[i].Align;
	...
	Object->Scaler = &CcrSummaryScaler;
	DialogRegisterScaler(Object);

	ShowWindow(GetDlgItem(hWnd, IDC_BUTTON_CCR_SUMMARY), SW_HIDE);
	return TRUE;
}
	...
	)
{
	...
	case WM_INITDIALOG:
		return CcrSummaryOnInitDialog(hWnd, uMsg, wp, lp);

	case WM_CLOSE:
		return CcrSummaryOnClose(hWnd, uMsg, wp, lp);

	case WM_ERASEBKGND:
		return CcrSummaryOnEraseBkgnd(hWnd, uMsg, wp, lp);

	case WM_NOTIFY:
		return CcrSummaryOnNotify(hWnd, uMsg, wp, lp);
	}
	...
	)
{
	...
	if(IDC_LIST_CCR_SUMMARY == pNmhdr->idFrom) {

		switch (pNmhdr->code) {
		case NM_CUSTOMDRAW:
			return CcrSummaryOnCustomDraw(Object, pNmhdr);
		}
